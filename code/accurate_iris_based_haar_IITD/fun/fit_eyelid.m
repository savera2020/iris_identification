function [coffs, im_eyelid] = fit_eyelid(im, ci, ri, cp, rp, range, position, offset, save_image)
...
if length(yy) < ri*0.3
    coffs = get_default_coff(ci, ri, position);
else
...
if save_image
    ...
    function coffs = get_default_coff(ci, ri, position)
        if strcmp(position, 'upper') == 1 % equal
